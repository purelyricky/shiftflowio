...
import { Users, Clock, AlertTriangle, CheckCircle, Search, ChevronLeft, ChevronRight, Info, MessageSquare, Send, Sliders } from 'lucide-react'
...
export function HrAttendanceTracking() {
  ...
  const [contactType, setContactType] = useState<'leader' | 'admin'>('leader')
  ...
  const handleContactAdmin = (worker: typeof initialAttendance[0]) => {
    setSelectedWorker(worker)
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <h1 className="text-3xl font-bold mb-8">HR Attendance Tracking</h1>
            ...
            <form onSubmit={handleSearch} className="flex items-center space-x-2 w-full sm:w-auto">
              <div className="relative flex-grow sm:flex-grow-0">
                ...
                <Input
                  ...
                  className="pl-8 pr-4 py-2 w-full sm:w-64 h-9 text-sm"
                />
              </div>
                    ...
                    <TableCell>
                      <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                        <Button
                          size="sm"
                          variant="outline"
                          className="bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 w-full sm:w-auto"
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          className="bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-700 w-full sm:w-auto"
                          onClick={() => handleContactAdmin(record)}
                        >
                          <MessageSquare className="h-4 w-4 mr-1" />
                          Admin
                        </Button>
      ...
      <Dialog open={contactModalOpen} onOpenChange={setContactModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Contact {contactType === 'leader' ? 'Leader' : 'Admin'}</DialogTitle>
            <DialogDescription>
